...
resource "aws_security_group" "kxs-master-sg" {
  name        = "kxs-master-sg"
  description = "Grupo de seguridad master del cluster"
  # vpc_id      = aws_vpc.kxs-vpc.id
  vpc_id = var.vpc_domotica
...
resource "aws_security_group" "kxs-worker-sg" {
  name        = "kxs-worker-sg"
  description = "Grupo de seguridad worker del cluster"
  #  vpc_id      = aws_vpc.kxs-vpc.id
  vpc_id = var.vpc_domotica
...
resource "aws_security_group" "kxs-mariadb-sg" {
  name        = "kxs-mariadb-sg"
  description = "Grupo de seguridad mariadb"
  # vpc_id      = aws_vpc.kxs-vpc.id
  vpc_id = var.vpc_domotica
